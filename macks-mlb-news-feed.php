<?php
/*
Plugin Name: Mack's MLB Baseball News Feed
Description: Lists the Latest from MLB Baseball
...
#
function widget_macks_mlb_news_feed() 
{
?>
<?php get_macks_mlb_news_feed(); ?>
<?php
}
#
# Function registers the plugin
#
function macks_mlb_news_feed_install()
{
register_sidebar_widget(__('Mack\'s MLB Baseball News Feed'), 'widget_macks_mlb_news_feed'); 
}
add_action("plugins_loaded", "macks_mlb_news_feed_install");
#
# Activation Hook
#
register_activation_hook(__FILE__,'macks_mlb_news_feed_activate');
#
# Deactivation Hook
#
register_deactivation_hook( __FILE__, 'macks_mlb_news_feed_remove' );
#
# Let's Install Mack's Plugin :)
#
function macks_mlb_news_feed_activate() 
{
add_option("macks_mlb_news_feed_width", "162", "Width of Feed", "yes");
add_option("macks_mlb_news_feed_height", "290", "Height Of Feed", "yes");
add_option("macks_mlb_news_feed_articles", "3", "Article Count", "yes");
add_option("mackdbwidget", "0", "Menu Status", "yes");
}
#
# Let's Remove Mack's Plugin :(
#
function macks_mlb_news_feed_remove() 
{
delete_option('macks_mlb_news_feed_width');
delete_option('macks_mlb_news_feed_height');
delete_option('macks_mlb_news_feed_articles');
...
**************************************************************************/
function mlb_fetch_rss_items( $num ) {
...
function mlb_MainFunction() {
			...
			# Get the RSS Feed
			#
			$rss_items = mlb_fetch_rss_items(3);
 ...
 */
function mlb_MainFunction_Init() {
	wp_add_dashboard_widget( 'mlb_MainFunction', __( 'Macks\'s MLB Baseball News' ), 'mlb_MainFunction', 'mlb_MainFunction_Setup' );
}
#
# The Setup Function like Yoast WP-SEO Plugin
#
function mlb_MainFunction_Setup() {
	    		wp_add_dashboard_widget( 'mack_db_widget' , 'The Latest From Mack' , array(&$this, 'mlb_MainFunction') );
		}
 
#
# This function sorts the order of dashboard boxes, like Yoast WP-SEO Plugin
#
function mlb_MainFunction_Order( $arr ) 
 			{
			#
			...
			if ($menu == 1)
			...
			else
				{
					if ( is_admin() ) 
						{
						if ( isset($wp_meta_boxes['dashboard']['normal']['core']['mlb_MainFunction']) ) 		{
						$mlb_MainFunction = $wp_meta_boxes['dashboard']['normal']['core']['mlb_MainFunction'];
						unset($wp_meta_boxes['dashboard']['normal']['core']['mlb_MainFunction']);
						if ( isset($wp_meta_boxes['dashboard']['side']['core']) ) 
							...
							$wp_meta_boxes['dashboard']['side']['core'][] = $mlb_MainFunction;
 ...
 */
add_action('wp_dashboard_setup', 'mlb_MainFunction_Init');
add_action('wp_dashboard_setup', 'mlb_MainFunction_Order');
add_filter('wp_dashboard_setup', 'mlb_MainFunction_Init');
add_filter('wp_dashboard_setup', 'mlb_MainFunction_Order');
#
# Menu for the Plugin, adding options to WP
#
if ( is_admin() )
	{
	add_action('admin_menu', 'macks_mlb_news_feed_admin_menu');

	function macks_mlb_news_feed_admin_menu() {
		add_options_page('Mack\'s MLB Baseball News Feed', 'Mack\'s MLB Baseball News Feed Settings', 'administrator', 'macks-mlb-news-feed.php', 'macks_mlb_news_feed_plugin_page');
		}
	}
#
# The actual code that calls the feed dynamically
#
function get_macks_mlb_news_feed()
{
...
#
$width = get_option('macks_mlb_news_feed_width');
$height = get_option('macks_mlb_news_feed_height');
$articles = get_option('macks_mlb_news_feed_articles');
#
# The feed URL where we can get the actual news and customized size
#
$feedurl = "http://www.jawer.org/wp-plugins/macks-mlb-news-feed/macks-mlb-news-creator.php?mywidth=$width&myheight=$height&articles=$articles";
#
# Use Magpie to Get the Feed
#
$rss = macks_mlb_news_feed_fetch_rss( $feedurl );
...
#
function macks_mlb_news_feed_plugin_page() { ?>
<div class="postbox-container" style="width:70%; height:40%;"> 
<div class="metabox-holder">	
	<div class="meta-box-sortables"> 
	<div id="general-settings" class="postbox"> 
	<h3 class="hndle"><span>Mack's MLB Baseball Feed Settings</span></h3> 
	<div class="inside">

<!-- -->
<img src="../wp-content/plugins/macks-mlb-baseball-news-feed/mack.JPG" width="91" height="64" alt="Mack" longdesc="http://www.wordpress.org" />
...
<?php wp_nonce_field('update-options'); ?>
	<input type="Text" name="macks_mlb_news_feed_width" id="macks_mlb_news_feed_width" value="<?php echo get_option('macks_mlb_news_feed_width'); ?>" />
      <br />
      <input type="hidden" name="action" value="update" />
   	  <input type="hidden" name="page_options" value="macks_mlb_news_feed_width" />
      <input name="macks_default_width" type="hidden" value="162" />
	  <input type="button" value="Default Width" onClick="document.setwidth.macks_mlb_news_feed_width.value=document.setwidth.macks_default_width.value">
  	  <p>
...
<?php wp_nonce_field('update-options'); ?>
	<input type="Text" name="macks_mlb_news_feed_height" id="macks_mlb_news_feed_height" value="<?php echo get_option('macks_mlb_news_feed_height'); ?>" />
      <br />
      <input type="hidden" name="action" value="update" />
   	  <input type="hidden" name="page_options" value="macks_mlb_news_feed_height" />
      <input name="macks_default_height" type="hidden" value="290" />
	  <input type="button" value="Default Height" onClick="document.setheight.macks_mlb_news_feed_height.value=document.setheight.macks_default_height.value">
...
<br />
<br />
Select the number of articles the feed will display. Choose up to 10. <br />Currently, <strong><?php echo get_option('macks_mlb_news_feed_articles'); ?></strong> are being displayed.
<br />
<form name="setarticles" method="post" action="options.php">
<?php wp_nonce_field('update-options'); ?>
	<select name="macks_mlb_news_feed_articles" id="macks_mlb_news_feed_articles" value="<?php echo get_option('macks_mlb_news_feed_articles'); ?>" />
    ...
    <input type="hidden" name="action" value="update" />
   	  <input type="hidden" name="page_options" value="macks_mlb_news_feed_articles" />
  	  <p>
      <input type="submit" value="<?php _e('Save Changes') ?>" />
     </p>
</form>
...
<div class="metabox-holder">	
	...
	<div class="inside">
    <p>
<?php echo get_macks_mlb_news_feed(); ?>
</p>
</div>
</div>
</div>
